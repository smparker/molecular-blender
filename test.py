...
class TestPySCFFunctions(unittest.TestCase):
    """Testing molden files generated by PySCF

    Functions in this test correspond to polarization, not orbitals"""
    def setUp(self):
        """Setup function"""
        self.geo = mb.importers.molecule_from_file(
            'examples/phenanthroline_pbe0_ris.molden', {})
        self.orbitals = mb.orbitals.MOData.from_dict(self.geo)
        self.xyz = np.linspace(-15, 15, 64, dtype=np.float32)
        self.dvol = (self.xyz[1] - self.xyz[0])**3

    def test_read_molden(self):
        """Test to read molden"""
        self.assertEqual(len(self.geo["atoms"]), 22)

    def test_polarizability(self):
        """Test to verify polarizability"""
        polar = self.orbitals.get_orbital(1)
        ...
        # integrating polarization * r should give the polarizability
        alphax = np.zeros([3], dtype=np.float32)
        def integrate_polarizability(pxyz, xyz, dvol):
            alphax = np.zeros([3], dtype=np.float32)
            for i in range(3):
                # first sum along the other two axes
                other_axes = tuple([ x for x in range(3) if x != i ])
                partial = np.sum(pxyz, axis=other_axes)
                # then integrate along the remaining axis
                integrate = -np.sum(partial * xyz) * dvol
                alphax[i] = integrate
            return alphax

        alphax = integrate_polarizability(pxyz, self.xyz, self.dvol)

        self.assertAlmostEqual(alphax[0], 228.4, delta=0.5)
        self.assertAlmostEqual(alphax[1], 0.0, delta=0.5)
        self.assertAlmostEqual(alphax[2], 0.0, delta=0.5)

        # test the polarizability of the second orbital
        polar = self.orbitals.get_orbital(2)
        pxyz = polar.box_values(self.xyz, self.xyz, self.xyz)
        alphay = integrate_polarizability(pxyz, self.xyz, self.dvol)
        self.assertAlmostEqual(alphay[0], 0.0, delta=0.5)
        self.assertAlmostEqual(alphay[1], 0.0, delta=0.5)
        self.assertAlmostEqual(alphay[2], 0.0, delta=0.5)

        # test the polarizability of the third orbital
        polar = self.orbitals.get_orbital(3)
        pxyz = polar.box_values(self.xyz, self.xyz, self.xyz)
        alphaz = integrate_polarizability(pxyz, self.xyz, self.dvol)
        self.assertAlmostEqual(alphaz[0], 0.0, delta=0.5)
        self.assertAlmostEqual(alphaz[1], 0.0, delta=0.5)
        self.assertAlmostEqual(alphaz[2], 150.7, delta=0.5)
